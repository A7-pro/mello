...
bot_data = {
    ...
    "pornography_keywords": [
        ...
        "نيك ميت", "نكروفيليا", "نيك اجباري", "اغتصاب قسري", "هتك عرض", "تحرش جنسي", "اختصاب", "مشتهية", "مبادل",
    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, 'r') as f:
                ...
                if "group_settings" in loaded_data: bot_data["group_settings"].update(loaded_data["group_settings"])
                if "protection_settings" in loaded_data: bot_data["protection_settings"].update(loaded_data["protection_settings"])
                if "user_messages_history" in loaded_data: bot_data["user_messages_history"].update(loaded_data["user_messages_history"])
                if "bank_accounts" in loaded_data: bot_data["bank_accounts"].update(loaded_data["bank_accounts"])
                if "user_properties" in loaded_data: bot_data["user_properties"].update(loaded_data["user_properties"])
                if "stock_market" in loaded_data: bot_data["stock_market"].update(loaded_data["stock_market"])
                if "loans" in loaded_data: bot_data["loans"].update(loaded_data["loans"])
                if "insurance" in loaded_data: bot_data["insurance"].update(loaded_data["insurance"])
                if "vip_cards" in loaded_data: bot_data["vip_cards"].update(loaded_data["vip_cards"])
                if "user_jobs" in loaded_data: bot_data["user_jobs"].update(loaded_data["user_jobs"])
                if "gang_leaders" in loaded_data: bot_data["gang_leaders"].update(loaded_data["gang_leaders"])
                if "banned_users" in loaded_data: bot_data["banned_users"].update(loaded_data["banned_users"])
                if "reports" in loaded_data: bot_data["reports"].update(loaded_data["reports"])
                if "user_reported_count" in loaded_data: bot_data["user_reported_count"].update(loaded_data["user_reported_count"])
                if "report_limits" in loaded_data: bot_data["report_limits"].update(loaded_data["report_limits"])
                ...
                if "user_activity" in loaded_data: bot_data["user_activity"].update(loaded_data["user_activity"])
                if "group_activity" in loaded_data: bot_data["group_activity"].update(loaded_data["group_activity"])
                if "user_achievements" in loaded_data: bot_data["user_achievements"].update(loaded_data["user_achievements"])
                if "secondary_devs" in loaded_data: bot_data["secondary_devs"].update(loaded_data["secondary_devs"])
                ...
                else: bot_data["pornography_keywords"] = []
                ...
                user_reported_count = bot_data["user_reported_count"]
                report_limits = bot_data["report_limits"]
                linked_channels = bot_data["linked_channels"]
                user_activity = bot_data["user_activity"]
                group_activity = bot_data["group_activity"]
                user_achievements = bot_data["user_achievements"]
                secondary_devs = bot_data["secondary_devs"]
...
def prison_check_handler(client, message):
    ...
    # Apply check only to text messages that could be commands or trigger actions
    if message.text:
        # List command prefixes that should be blocked by prison
        command_prefixes = [
            ...
            "قائمة المنع", "ردود القروب", "اضف رد", "حذف رد", "اضف امر", "حذف امر", "اوامري",
            "ايديي", "ايدي" # Include ID commands
            # AI interaction is removed
        ]
        # Check if the message text starts with any of the command prefixes
        is_command = False
        text_lower = message.text.lower().split(maxsplit=1)[0] # Get the first word/command

        if text_lower.startswith('/'): # Also check slash commands
             is_command = True
        elif text_lower in command_prefixes:
             is_command = True
        # Add check for commands like "اضف رد", "حذف رد", "اضف كلمة", "حذف كلمة", etc. which have two words
        elif len(message.text.split()) > 1:
             two_word_command = " ".join(message.text.lower().split(maxsplit=2)[:2])
             if two_word_command in ["اضف كلمة", "حذف كلمة", "اضف رد", "حذف رد", "اضف امر", "حذف امر", "ضع بلاغات", "رفع قيود", "تنزيل الكل"]:
                  is_command = True


        # Only block if it looks like a command and user is in prison
        if is_command:
             if is_in_prison(user_id):
                 remaining_time = int(banned_users[user_id] - time.time())
                 minutes = remaining_time // 60
                 seconds = remaining_time % 60
                 # Clear expired prison status if time is up
                 if remaining_time <= 0:
                      del banned_users[user_id]
                      save_data()
                      # Let the command proceed after clearing prison status
                      return # Do not stop propagation, let the command handler run
                 else:
                      # Still in prison
                      message.reply_text(f"⏳ **أنت مسجون حالياً!** تبقى لك {minutes} دقيقة و {seconds} ثانية. لا يمكنك استخدام الأوامر.", quote=True)
                      message.stop_propagation() # Stop other handlers from processing this message
                      return

    # Continue propagation if not in prison or not a relevant message/command
...
def promote_user_cmd(client, message):
    # Prison check is handled by prison_check_handler above
...
def lock_unlock_commands_cmd(client, message):
    ...
    command_lookup = {cmd.split()[0]: cmd for cmd in valid_lockable_commands}
    ...
    elif command_or_feature in valid_lockable_commands:
         command_or_feature_key = command_or_feature # Use the single word command as the key
    else:
         command_or_feature_key = None # Not a valid lockable command


    if command_or_feature_key is None:
         return message.reply_text(f"❌ **الأمر/الميزة '{command_or_feature}' غير قابل للقفل/الفتح بهذا الأمر.**\nالأوامر المتاحة للقفل/الفتح: " + " - ".join(valid_lockable_commands), quote=True)
...
def is_command_locked(chat_id_str, command_name):
     ...
     full_command_name = command_name.lower()
     # For single-word checks, just use the word
     return chat_id_str in group_settings and group_settings[chat_id_str].get(full_command_name, False)
            ...
            if len(parts) == 2: # Ensure it's exactly "مسح [number]"
                 try:
                     ...
                     try:
                          ...
                          # If deleted_count is 0, it means the command message wasn't deleted either.
                          if deleted_count == 0:
    ...
    # مسح الأوامر المضافة (Clear group-specific custom commands)
        ...
        if chat_id in group_settings and "custom_commands" in group_settings[chat_id]:
            ...
            if chat_id in group_settings and not group_settings[chat_id]["custom_commands"]:
                 pass # Keep the key and empty dict
            save_data() # Save data
...
def bank_property_job_prison_system_cmd(client, message):
    ...
    if text == "انشاء حساب بنكي":
        if user_id in bank_accounts:
            return message.reply_text(f"🚫 **لديك حساب بنكي بالفعل! رصيدك الحالي: {bank_accounts[user_id]}💰**", quote=True)
    ...
    bank_commands_needed_account = ["رصيدي", "راتبي", "بخشيش", "استثمار", "العجلة",
                                     ...
                                     "انضم رئيس عصابة", "ترجل", "سجن", "سجني", "سداد ديوني"]
    ...
    # --- 🚔 نظام الشرطة والعصابات والسجن 🚔 ---
        ...
        # The prison check handler at the start should prevent commands if in prison.
        # This specific command should work even if in prison to check status.
        # The prison_check_handler needs to be modified to allow 'سجني' specifically.
        # Alternatively, handle the prison check here and bypass the main handler for this command.
        # Let's handle it here for this specific command.
...
def track_activity_handler(client, message):
    ...
    if points_gained > 0:
         ...
         for threshold, achievement_name in achievements.items():
...
def show_commands_cmd(client, message):
     ...
     if text == "الأوامر" or text == "/help":
         ...
         try:
              ...
              bot_username = "البوت" # Fallback if cannot get username
...
- `رفع [الرتبة] بالرد/المعرف/الآيدي`: لرفع رتبة عضو في البوت (الرتب: {", ".join(rank_display_names.values())}).
...
- `ضع قوانين [القوانين]`: لتحديد قوانين القروب.
- `اضف رابط = [الرابط]`: لتحديد رابط القروب.
...
def add_custom_command_cmd(client, message):
    ...
    built_in_commands = [
        "رفع", "تنزيل", "رتبتي", "كشف", "قفل", "فتح", "حظر", "الغاء", "طرد",
        "كتم", "تقييد", "فك", "رفع", "طرد", "طرد", "كشف", "القوانين", "ضع", "الرابط", "اضف",
        "المالك", "المدراء", "الادمن", "المشرفين", "المميزين", "اضف", "حذف",
        "مسح", "انشاء", "رصيدي", "راتبي", "بخشيش", "استثمار", "العجلة",
        "شراء", "بيع", "ممتلكاتي", "انضم", "ترجل", "سجن", "سجني", "سداد", "بلاغ", "ضع", "ربط",
        "توب", "صفر", "صفر", "اضف", "حذف",
        "قائمة", "ردود", "اضف", "حذف", "اضف", "حذف",
        "اوامري", "ايديي", "ايدي", "/help", "/start", "/panel" # Add common aliases
    ]
    # Check if the custom trigger is one of the main command prefixes or a slash command
